...
arduino = serial.Serial(port='/dev/ttyUSB0', baudrate=115200, timeout=1)  # Chỉnh cổng phù hợp
...
class GoalAndOdomLogger:
    ...
    def odom_callback(self, msg):
        rospy.loginfo("Đã nhận dữ liệu từ /odom")
        ...
        rospy.loginfo("Odom: x=%.3f, y=%.3f, theta=%.3f rad", position.x, position.y, yaw)
        rospy.loginfo("Sai số: Δx=%.3f, Δy=%.3f, Δθ=%.3f rad", ex, ey, eth)

    # Gửi dữ liệu qua Serial nếu kết nối mở
        if self.serial_conn and self.serial_conn.is_open:
            data_to_send = f"{ex:.3f},{ey:.3f},{eth:.3f}\n"
...
try:
    while True:
        ...
        time.sleep(0.5)  # Điều chỉnh tốc độ gửi (0.5 giây/lần)
